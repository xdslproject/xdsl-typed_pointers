...
class GenerateKernelsAndBuffersCode(RewritePattern):
    ...
    def match_and_rewrite(self, node_op: dataflow.Node, rewriter: PatternRewriter):
        ...
        self.create_kernel[0] += f"cl_kernel node_0_kernel = clCreateKernel(program, \"{node_name}\", &err);\n\t"
        ...
        if node_name in self.depends:
            self.enqueue_kernel[0] += f"err = clEnqueueTask(commands, {node_name}_kernel, {len(self.depends[node_name])}, {node_name}_waitlist, &{node_name}_finish);\n\t"
        else:
            self.enqueue_kernel[0] += f"err = clEnqueueTask(commands, {node_name}_kernel, 0, NULL, &{node_name}_finish);\n\t"
